...
Ecoli_growth_data <- read_csv('Putida_in_C6_growthcurves_1.csv') %>% 
  pivot_longer(cols = -c(Strain, Inducer, Replicate, Time),
               names_to = 'Inducers',
               values_to = 'OD') %>%
  filter(OD!=0) %>%
  mutate(name = ifelse(Inducers == 'WT', 'WT', 
                       ifelse(Inducer == 'C6 HSL', paste0(Inducers, 'uM C6 HSL'),
                              ifelse(Inducer == 'Fn', paste0('pSR2022, ', Inducers, ' ng/mL aTc'),
...
Ecoli_growth_summary <- Ecoli_growth_data %>% 
  group_by(Strain, Inducer, Inducers, name, Time) %>% 
...
test_data <- filter(Ecoli_growth_data, Strain == 'KT2440', 
                    Inducers == '0.1', Replicate == 1, Inducer == 'C6 HSL')
...
all_fits <- all_easylinear(OD ~ Time | Strain + Inducer + Replicate + Inducers, 
                           data = Ecoli_growth_data, h=3)
...
for (strain in unique(Ecoli_growth_data$Strain)) {
  for (system in unique(Ecoli_growth_data$Inducer)) {
...
exp_fit_params_summary  <- exp_fit_params %>% 
  group_by(Strain, Inducer, Inducers, name) %>% 
...
for (system in unique(exp_fits_summary$Inducer)) {
  data_summary <- filter(exp_fits_summary, Inducer == system)
  data <- filter(exp_fit_params, Inducer == system)
  clrs <- if (system == 'Fn') Fn_clrs else if (system == 'Lb') Lb_clrs else Sp_clrs
...
for (strain in c('KT2440', 'pJB1000')) {
 ...
 for (system in unique(exp_fits_summary$Inducer)) {
   ...
   # }
   # text_labels <- filter(text_tibble, Inducer == system)
   ...
   # }
   plot
   plotname <- paste0('~/Bio 676/Quarto/AndrewsLab/images/', paste(sym(system), 'mu_barplots_v2.3.svg', sep = '_'))
...
growth_summary_4hr <- growth_data_4hr %>% 
  group_by(Strain, Inducer, Time, Inducers, name) %>% 
  summarize(OD_mean = mean(OD),
            OD_sd = sd(OD),
            n_reps = n())
...
#   for (system in unique(growth_data_4hr$Inducer)) {
...
growth_summary_4hr <- growth_summary_4hr %>% 
  left_join(rename(t_tests_OD, Inducers = Set), 
            by = c('Strain','Inducer', 'Inducers')) %>% 
  mutate(label = ifelse(od_pval >= 0.05, 'ns',
                        ifelse(od_pval > 0.01, '*',
                               ifelse(od_pval > 0.001, '**', '***'))))
growth_summary_4hr$Strain <- factor(growth_summary_4hr$Strain, 
                                    levels = c('KT2440', 'pJB1000'))

# Make a plot of the OD data for each CRISPRi system
# for (system in unique(growth_summary_4hr$Inducer)) {
#   data_summary <- filter(growth_summary_4hr, Inducer == system)
#   data <- filter(growth_data_4hr, Inducer == system)
#   clrs <- if (system == 'Fn') Fn_clrs else if (system == 'Lb') Lb_clrs else Sp_clrs
...
#             by = c('Strain', 'Inducer', 'Inducers', 'name')) %>%
#   group_by(Strain, Inducer) %>% 
...
# 
# write_csv(growth_sum_stats, '~/Bio 676/Quarto/AndrewsLab/stat_results/growth_rate_stats.csv')
...
for (strain in unique(exp_fit_params$Strain)) {
  for (conc in c('0','0.25','2','4')) {
    ...
    anova_res <- aov(mu ~ Inducer, data = data)
    if (summary(anova_res)[[1]][["Pr(>F)"]][1] < 0.05) {
      tukey <- TukeyHSD(anova_res, 'Inducer')
      df <- data.frame(tukey$Inducer)
...
high_atc_data[high_atc_data$Inducer == 'Sp',]$Inducer <- 'Sp-dCas9'
high_atc_data[high_atc_data$Inducer == 'Fn',]$Inducer <- 'Fn-dCas12a'
high_atc_data[high_atc_data$Inducer == 'Lb',]$Inducer <- 'Lb-dCas12a'
high_atc_data$Inducer <- factor(high_atc_data$Inducer,
                                       levels = c('Sp-dCas9', 'Fn-dCas12a', 
                                                  'Lb-dCas12a'))
high_atc_pts <- filter(exp_fit_params, Inducers == '0')
high_atc_pts[high_atc_pts$Inducer == 'Sp',]$Inducer <- 'Sp-dCas9'
high_atc_pts[high_atc_pts$Inducer == 'Fn',]$Inducer <- 'Fn-dCas12a'
high_atc_pts[high_atc_pts$Inducer == 'Lb',]$Inducer <- 'Lb-dCas12a'
high_atc_pts$Inducer <- factor(high_atc_pts$Inducer,
                                       levels = c('Sp-dCas9', 'Fn-dCas12a', 
                                                  'Lb-dCas12a'))

toxicity_across_systems_plot <- ggplot(high_atc_data,
                                       aes(x = Strain, y = mu_avg, fill = Inducer)) +
  geom_bar(stat = 'identity', color = 'black',
           position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = (mu_avg - mu_sd), ymax = (mu_avg + mu_sd)),
                width = 0.2, position = position_dodge(width = 0.9)) +
  geom_point(data = high_atc_pts,
             mapping = aes(x = Strain, y = mu, color = Inducer),
...
text_tibble <- tibble(x = as.numeric(), 
                      ...
                      label = as.character())
i <- 0
for (strain in c('KT2400','pJB1000')) {
  data <- filter(high_atc_data, Strain == strain)
  n_conditions <- length(data$Inducer)
...
growth_corr_results <- tibble(Inducer = as.character(),
...
rel_mu <- ungroup(exp_fits_summary) %>% 
  select(Strain, Inducer, Inducers, mu_avg) %>%
  group_by(Strain, Inducer) %>% 
...
strains <- c('KT2440', 'pJB1000')
i <- 2
for (system in c('Sp', 'Fn', 'Lb')) {
  df <- filter(rel_mu, Inducer == system) %>% 
...
for (system in unique(rel_mu$Inducer)) {
  df <- filter(rel_mu, )
}
ggplot(rel_mu,
  ...
  base_cat_theme +
  facet_wrap(~Inducer, scales = 'free') +
  facet_theme
